...
def train_seld(config):
    ...
    # Dataset — replace with actual STARSS23 / DCASE2025 dataset loader
    log.info("NOTE: Using dummy data. Replace with STARSS23 / DCASE2025 DataLoader.")
    from torch.utils.data import TensorDataset
    dummy_audio   = torch.randn(64, 2, int(config.data.audio_sr * config.data.audio_duration))
    dummy_targets = torch.randn(64, config.model.N_max, config.model.num_seld_classes, 3)
    dataset = TensorDataset(dummy_audio, dummy_targets)
    ...
    while global_step < config.training.total_steps:
        for audio, target_xyz in loader:
